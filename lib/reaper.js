...
var Reaper = function(request, response, user){
	...
	var process = function(err, res){
		...
		if(_currentPage > getPageFromLink(res.meta.link)){
			_completed = true;
		} else{
			_currentPage = pLink.query.page; 
		}
		
		for (var i = 0; i < res.length; i++) {
			//console.log(res[i].name);
			_results.push({name: res[i].name, description: res[i].description , language: res[i].language, owner: res[i].owner.login });
		};
